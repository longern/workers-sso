...
function callbackURL(rootDomain) {
...
function isExternalCallback(url) {
  try {
...
async function startOAuth(provider, path, next) {
...
function accountList(session, accounts, config) {
...
function renderHome(session, accounts, config) {
...
async function boot() {
  const config = await fetch("/api/public-config", { credentials: "include" })
    .then((r) => r.json());
